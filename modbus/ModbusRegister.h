...
namespace Mb{
    ...
    class ModbusRegister{
        public:
            /**
             * @brief Construct a new ModbusRegister object
             *
             ...
             */
            explicit ModbusRegister(ModbusDevice* device_, int addr_, float factor_ = 1., std::string unit_ = "") :
                device(device_),
                addr(addr_),
                factor(factor_),
                unit(unit_),
            ...
            ModbusRegister(const ModbusRegister& other) = delete;
