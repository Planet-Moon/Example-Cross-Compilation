...
namespace Mb{
    ...
    void ModbusDevice::init(const char* ipAddress_, int port_)
    {
    ...
    void ModbusDevice::start_thread()
    {
        if(read_values_run == false){
            read_values_run = true;
            read_values_thread = std::make_unique<std::thread>(&ModbusDevice::read_values_task, this);
            #ifdef DEBUG
    ...
    bool ModbusDevice::connect(const char* ipAddress_, int port_)
    {
    ...
    void test_modbus()
    {
        ...
        std::cout << "read registers: " << read_registers << std::endl;
        modbus_close(mb);
        modbus_free(mb);
        for(int i=0; i<32; ++i){
            std::cout << "tab_reg[" << i << "]: " << tab_reg[i] << std::endl;
    ...
    bool ModbusDevice::read_all_registers()
    {
        return true;
    }

    void ModbusDevice::read_values_task()
    {
        #ifdef DEBUG
            std::cerr << "modbus connection " + ipAddress + ":" << port << " thread started"<< std::endl;
        #endif // DEBUG
    ...
    void ModbusDevice::update_connections_stats(char connection_state)
    {
        test_counter++;
        test_history += "|";
    ...
    void ModbusDevice::print_counters()
    {
